...
int main(int argc, char **argv)
{
    ...
    if (argc != 4)
    {
        fprintf(stderr, "Usage: %s NX NY [inject|restrict]\n", argv[0]);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }

    const int global_nx = atoi(argv[1]);
    const int global_ny = atoi(argv[2]);
    const int do_restrict = (strcmp(argv[3], "restrict") == 0);

    if (!do_restrict && strcmp(argv[3], "inject") != 0)
    {
    ...
    setup_2d_domain(px, py, mpi_rank, global_nx, global_ny, gs,
     ...
     * inject_var_2d fills all non-ghost child points (including physical
     * boundaries).  When restricting, restrict_var_2d then overwrites the
    ...
    if (do_restrict)
    {
         ...
         * be modified by restrict_var_2d.  Overwrite them now and verify
        ...
        if (child->domain.lower_x_rank == INVALID_RANK)
            for (int64_t jc = 0; jc < cny; jc++)
                child->vars[0]->val[gf_indx_2d(child, 0, jc)] = SENTINEL;
        if (child->domain.upper_x_rank == INVALID_RANK)
            for (int64_t jc = 0; jc < cny; jc++)
                child->vars[0]->val[gf_indx_2d(child, cnx - 1, jc)] = SENTINEL;
        if (child->domain.lower_y_rank == INVALID_RANK)
            for (int64_t ic = 0; ic < cnx; ic++)
                child->vars[0]->val[gf_indx_2d(child, ic, 0)] = SENTINEL;
        if (child->domain.upper_y_rank == INVALID_RANK)
            for (int64_t ic = 0; ic < cnx; ic++)
                child->vars[0]->val[gf_indx_2d(child, ic, cny - 1)] = SENTINEL;
        ...
        if (child->domain.lower_x_rank == INVALID_RANK)
            for (int64_t jc = 0; jc < cny; jc++)
                if (child->vars[0]->val[gf_indx_2d(child, 0, jc)] != SENTINEL)
                {
                    fprintf(stderr, "Rank %d: restrict touched lower-x boundary child point at jc=%lld\n",
                            mpi_rank, (long long)jc);
                    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
                }
        if (child->domain.upper_x_rank == INVALID_RANK)
            for (int64_t jc = 0; jc < cny; jc++)
                if (child->vars[0]->val[gf_indx_2d(child, cnx - 1, jc)] != SENTINEL)
                {
                    fprintf(stderr, "Rank %d: restrict touched upper-x boundary child point at jc=%lld\n",
                            mpi_rank, (long long)jc);
                    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
                }
        if (child->domain.lower_y_rank == INVALID_RANK)
            for (int64_t ic = 0; ic < cnx; ic++)
                if (child->vars[0]->val[gf_indx_2d(child, ic, 0)] != SENTINEL)
                {
                    fprintf(stderr, "Rank %d: restrict touched lower-y boundary child point at ic=%lld\n",
                            mpi_rank, (long long)ic);
                    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
                }
        if (child->domain.upper_y_rank == INVALID_RANK)
            for (int64_t ic = 0; ic < cnx; ic++)
                if (child->vars[0]->val[gf_indx_2d(child, ic, cny - 1)] != SENTINEL)
                {
                    fprintf(stderr, "Rank %d: restrict touched upper-y boundary child point at ic=%lld\n",
                            mpi_rank, (long long)ic);
                    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
                }

        /* Restore boundary cells from analytic f = 1.5x - 2y so that the
         * downstream sync and verify.py see correct values there. */
        if (child->domain.lower_x_rank == INVALID_RANK)
            for (int64_t jc = 0; jc < cny; jc++)
            {
                const double x = child->x0 + 0 * child->dx;
                const double y = child->y0 + jc * child->dy;
                child->vars[0]->val[gf_indx_2d(child, 0, jc)] = 1.5 * x - 2.0 * y;
            }
        if (child->domain.upper_x_rank == INVALID_RANK)
            for (int64_t jc = 0; jc < cny; jc++)
            {
                ...
                const double y = child->y0 + jc * child->dy;
                child->vars[0]->val[gf_indx_2d(child, cnx - 1, jc)] = 1.5 * x - 2.0 * y;
            }
        if (child->domain.lower_y_rank == INVALID_RANK)
            for (int64_t ic = 0; ic < cnx; ic++)
            {
                ...
                child->vars[0]->val[gf_indx_2d(child, ic, 0)] = 1.5 * x - 2.0 * y;
            }
        if (child->domain.upper_y_rank == INVALID_RANK)
            for (int64_t ic = 0; ic < cnx; ic++)
            {
                ...
                child->vars[0]->val[gf_indx_2d(child, ic, cny - 1)] = 1.5 * x - 2.0 * y;
            }
    }
