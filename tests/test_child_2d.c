...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "Rank %d: ngfs_2d_create_hierarchy failed\n", mpi_rank);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    ...
    if (ngfs_2d_create_child(&gfs, min_cells) != -1)
    {
    ...
    {
        struct ngfs_2d *par = &gfs;
        int level = 0;
        while (par->child != NULL)
        {
            struct ngfs_2d *chd = par->child;
            level++;
    ...
    struct ngfs_2d *leaf = &gfs;
    while (leaf->child)
        leaf = leaf->child;
    if (ngfs_2d_create_child(leaf, min_cells) != 1)
    {
