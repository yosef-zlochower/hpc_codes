...
*     multigrid hierarchy.  The child grid has half the cell count in
*     each direction (grid points = cells/2 + 1).  Coarsening is
*     rejected if any direction has an odd cell count or if the child
*     would have fewer than min_cells_per_direction interior cells on
*     any rank.
* Input Variables:
...
*     variable in 3D.  Each coarse-grid point that coincides with a
*     fine-grid point (child spacing = 2 * parent spacing) receives
*     the fine-grid value directly.  Covers all non-ghost child
*     points (including physical boundary points).
* Input Variables:
