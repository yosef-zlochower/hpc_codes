...
the Week-11 \texttt{Maxwell\_Penalty} reference: the physics and the
...
\texttt{Maxwell\_Penalty} from Week 11.  The physics, the
...
\begin{itemize}[itemsep=2pt,topsep=2pt]
  \item \texttt{../Week11/Maxwell\_Penalty/CLAUDE.md} --- parent code
        notes (TOML schema, source types, lens demo, convergence
        rates, SBP--SAT sign convention warnings).
  \item \texttt{../Week11/Maxwell\_Penalty/doc/documentation.tex}
  ...
  \item \texttt{../Week11/Maxwell/Develop\_Penalty.md} --- continuum
        derivation of the SBP--SAT penalty for the extended Maxwell
        system.
...
inject the wave.  The hand-written sign convention in
\texttt{Maxwell/src/PenaltyImplementation.md} contains errors of
exactly this form; the SymPy-emitted macros in
\texttt{simple\_maxwell.h} are the authoritative version, derived
from scratch by SymPy diagonalization of $M_a$ and verified
against the traveling-wave check above.
